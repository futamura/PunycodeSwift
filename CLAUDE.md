...
- Simulator test jobs flake in two ways, neither caused by the code: they abort with exit code 134 during teardown after every test passed, or they hang and get cancelled on the job's `timeout-minutes` (15 for the simulator matrix, 10 for the macOS, Catalyst and SPM jobs; `lint_code` sets none). `gh pr checks` reports both as `fail`; `gh api repos/futamura/PunycodeSwift/actions/jobs/<job-id>` tells them apart, since a timeout shows `"conclusion": "cancelled"`. Rerun the failed jobs (`gh run rerun <id> --failed`) instead of debugging the code.
