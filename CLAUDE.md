...
The package is listed at https://swiftpackageindex.com/futamura/PunycodeSwift, and `.spi.yml` controls what SPI does with it.

- `documentation_targets` has SPI build and host the DocC site. `custom_documentation_parameters: [--include-extended-types]` is required alongside it — the public API is entirely extensions on `String`/`Substring`, so without that flag the site SPI generates is almost empty. It is the swift-docc-plugin equivalent of the `-emit-extension-block-symbols` flag in `scripts/gen_docs.sh`. The GitHub Pages site is independent; both stay available.
- `metadata.authors` overrides the author line SPI derives from the commit history. SPI prints that value **verbatim**, capped at 200 characters — the `Written by ` prefix and the full stop are added only to the derived line — so the value has to be a complete sentence.
- Edits to `.spi.yml` on `main` do not appear straight away. `Analyze.throttle()` keeps the stored default-branch version until the commit SPI already holds is 24 hours old (`Constants.branchVersionRefreshDelay`), and `version.spiManifest` is only written during that pass. Merging again neither resets nor shortens the timer, and when the refresh does fire it picks up every pending change at once. Tag analysis skips the throttle, which is why a release shows up the same day while the author line lags. Waiting is the only remedy — this is designed behaviour, not a bug worth reporting.
- SPI's web pages return 403 to curl/WebFetch (user-agent block), so check them in a browser. The badge JSON at `https://swiftpackageindex.com/api/packages/futamura/PunycodeSwift/badge?type=platforms` is `no-store` and always answers fresh; img.shields.io caches the same data for six hours (`cacheSeconds: 21600`), so the README badges lag behind the site.
...
- Simulator test jobs flake in two ways, neither caused by the code: they abort with exit code 134 during teardown after every test passed, or they hang and get cancelled on the job's `timeout-minutes` (15 for the simulator matrix, 10 for every other job). `gh pr checks` reports both as `fail`; `gh api repos/futamura/PunycodeSwift/actions/jobs/<job-id>` tells them apart, since a timeout shows `"conclusion": "cancelled"`. Rerun the failed jobs (`gh run rerun <id> --failed`) instead of debugging the code.
